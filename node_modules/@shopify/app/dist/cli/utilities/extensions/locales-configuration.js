...
const CHECKOUT_UI_EXTENSION_KEY = 'checkout_ui';
const MissingDefaultLanguageError = () => {
    return new error.Abort(`Missing default language in ${CHECKOUT_UI_EXTENSION_KEY} configuration`, 'Make sure to have a {locale}.default.json file in your locales directory');
};
const BigBundleError = () => {
    return new error.Abort(`Error loading ${CHECKOUT_UI_EXTENSION_KEY}`, `Total size of all locale files must be less than ${L10N_BUNDLE_SIZE_LIMIT}`);
};
const BigFileError = (filename) => {
    return new error.Abort(`Error loading ${CHECKOUT_UI_EXTENSION_KEY}`, `Locale file ${filename} size must be less than ${L10N_FILE_SIZE_LIMIT}`);
};
...
const MultipleDefaultError = () => {
    return new error.Abort(`Error loading ${CHECKOUT_UI_EXTENSION_KEY}`, `There must be one (and only one) locale identified as the default locale: e.g. "en.default.json"`);
};
export async function loadLocalesConfig(extensionPath) {
