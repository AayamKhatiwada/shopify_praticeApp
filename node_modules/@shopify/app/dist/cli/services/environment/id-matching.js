...
export async function automaticMatchmaking(localSources, remoteSources, identifiers, remoteIdField) {
    ...
    const existsRemotely = (local) => {
        return Boolean(remoteSources.find((remote) => remote[remoteIdField] === identifiers[local.localIdentifier] && remote.type === local.graphQLType));
    };
    ...
    const remoteUnmatched = pending.remote.filter((remote) => !pending.local.map((local) => local.graphQLType).includes(remote.type));
