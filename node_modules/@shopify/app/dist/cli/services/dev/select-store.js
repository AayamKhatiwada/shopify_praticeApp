...
const InvalidStore = (storeName) => {
    return new error.Abort(`The store you specified (${storeName}) is not a dev store`, 'Run dev --reset and select an eligible dev store.');
};
 ...
 */
export async function selectStore(stores, org, token, cachedStoreName) {
    if (cachedStoreName) {
...
export async function convertToTestStoreIfNeeded(store, org, token) {
     ...
     */
    if (environment.service.isSpinEnvironment() && environment.local.firstPartyDev())
        return;
