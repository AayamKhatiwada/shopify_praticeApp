...
import { extensions, getExtensionOutputConfig, limitedExtensions, isUiExtensionType, isThemeExtensionType, isFunctionExtensionType, functionExtensionTemplates, extensionTypeCategory, extensionTypeIsGated, } from '../../../constants.js';
...
import { mapExternalExtensionTypeToExtensionType, mapExtensionTypesToExternalExtensionTypes, mapExtensionTypeToExternalExtensionType, } from '../../../utilities/extensions/name-mapper.js';
...
export default class AppScaffoldExtension extends Command {
    ...
    async run() {
        ...
        flags.type = mapExternalExtensionTypeToExtensionType(flags.type);
        ...
        const formattedSuccessfulMessage = this.formatSuccessfulRunMessage(promptAnswers.extensionType, path.relative(app.directory, extensionDirectory), app.packageManager);
    ...
    async validateExtensionType(type) {
        ...
        if (!supportedExtensions.includes(type)) {
            throw new error.Abort(`The following extension types are supported: ${mapExtensionTypesToExternalExtensionTypes(supportedExtensions).join(', ')}`);
        }
    }
    /**
    ...
    validateExtensionTypeLimit(app, type) {
        if (type && this.limitedExtensionsAlreadyScaffolded(app).includes(type)) {
            throw new error.Abort('Invalid extension type', `You can only scaffold one extension of type ${mapExtensionTypeToExternalExtensionType(type)} per app`);
        }
    }
    ...
    formatSuccessfulRunMessage(extensionType, extensionDirectory, depndencyManager) {
        ...
        if (extensionOutputConfig.helpURL) {
            outputTokens.push(output.content `  For more details, see the ${output.token.link('docs', extensionOutputConfig.helpURL)} ✨`
                .value);
        }
