...
let loadedExtensionSpecs;
let loadedFunctionSpecs;
export async function allExtensionSpecifications() {
    if (loadedExtensionSpecs)
        return loadedExtensionSpecs;
    const registrations = await loadSpecs('extensions-specifications');
    // eslint-disable-next-line require-atomic-updates
    loadedExtensionSpecs = registrations;
    return registrations;
}
export async function allFunctionSpecifications() {
    if (loadedFunctionSpecs)
        return loadedFunctionSpecs;
    const registrations = await loadSpecs('function-specifications');
    // eslint-disable-next-line require-atomic-updates
    loadedFunctionSpecs = registrations;
    return registrations;
}
async function loadSpecs(directoryName) {
