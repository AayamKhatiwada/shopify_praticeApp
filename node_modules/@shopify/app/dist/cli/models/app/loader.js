import { FunctionExtensionConfigurationSchema, FunctionExtensionMetadataSchema, ThemeExtensionConfigurationSchema, UIExtensionConfigurationSupportedSchema, } from './extensions.js';
...
import { error, file, id, path, string, toml, output } from '@shopify/cli-kit';
...
export async function load(directory, mode = 'strict') {
...
class AppLoader {
    ...
    async loadWebs() {
        ...
        const webTomlsInStandardLocation = await path.glob(path.join(this.appDirectory, `web/**/${configurationFileNames.web}`));
    ...
    async loadUIExtensions(extensionDirectories) {
        ...
        const extensions = configPaths.map(async (configurationPath) => {
            ...
            const configurationSupported = await this.parseConfigurationFile(UIExtensionConfigurationSupportedSchema, configurationPath);
            const configuration = {
                ...configurationSupported,
                type: mapUIExternalExtensionTypeToUIExtensionType(configurationSupported.type),
            };
            const entrySourceFilePath = (await Promise.all(['index']
                ...
                .map(async (sourcePath) => ((await file.exists(sourcePath)) ? sourcePath : undefined)))).find((sourcePath) => sourcePath !== undefined);
            if (!entrySourceFilePath) {
                this.abortOrReport(output.content `Couldn't find an index.{js,jsx,ts,tsx} file in the directories ${output.token.path(directory)} or ${output.token.path(path.join(directory, 'src'))}`, undefined, directory);
            }
            return {
